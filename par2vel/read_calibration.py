class Calibration_image(object):
    def __init__(self, img_name):
        """The Calibration_image is called as Calibration_image(img_name), where
        img_name is the path to the calibration image. 
        The purpose of this object is to read an image of a carthesian grid, 
        where each point is represented by a dot. Call the object in order to open
        an image.
        It may be necessary to do some preleminary picture processing in order to
        eliminate shadows.
    ...
    def find_ellipses(self, edge_dist = [15, 15, 15, 15]):
        ...
        The optional argument edge_dist can be used to determine how close to the edge
        the centerpoints may lie. The distances are defined in pixels on the image plane
        
        >>> self.find_ellipses(edge_dist = [d_top, d_bottom, d_left, d_right])
        """

        from cv2 import Canny, findContours, fitEllipse, RETR_EXTERNAL, CHAIN_APPROX_NONE
        ...
        for ind, cont in enumerate(contours):
            ...
            if elpsa[ind][0][0] > edge_dist[2] and elpsa[ind][0][0] < self.shape[1] - edge_dist[3] and\
               elpsa[ind][0][1] > edge_dist[0] and elpsa[ind][0][1] < self.shape[0] - edge_dist[1]:
    ...
    def center_axis(self):
        ...
        self.center = self.closest_point(array([self.selected_x[0], self.selected_y[0]]))
        x_axis = self.closest_point(array([self.selected_x[1], self.selected_y[0]]))
        y_axis = self.closest_point(array([self.selected_x[2], self.selected_y[2]]))
    ...
    def object_coordinates(self, X_spacing = [1, 1], pixeltol = 20):
        ...
        arguments:
        - X_spacing, is a list or float, that defines the space between two dots
        in physical space
        - Pixeltol, how close two points have to be to each other, before it can
        be assumed that it is the same point.

        >>> self.object_coordinates(X_spacing = opt., pixeltol = opt.)
        """

        from numpy import vstack, ones, isclose, where, arange, sort, zeros, array
        ...
        x_axis = self.x[0:2, isclose(test_on_x, self.x[1], atol = pixeltol, rtol = 0)]
        y_axis = self.x[0:2, isclose(test_on_y, self.x[1], atol = pixeltol, rtol = 0)]
        ...
        ind_center = [int(where((x_axis[0] == self.center[0]) * \
                 (x_axis[1] == self.center[1]))[0][0]), int(where((y_axis[0] == self.center[0]) \
        ...
        Y_axis = vstack((zeros(len(y_axis[0])), -arange(len(y_axis[0])) + \
                 where((y_axis[0] == self.center[0]) * (y_axis[1] == self.center[1]))[0][0]))
        ...
        # The points along the x and y axis are now defined, the rest can now be filled out:
        """
        ...
        for i in X_axis[0, X_axis[0] > 0]:
            ...
            for j in sort(Y_axis[1, Y_axis[1] > 0]):
                j = int(j)
                X_grid[0:2, ind_center[1] - j, ind_center[0] + i] = array([i, j])
                x_point = X_grid[2:4, ind_center[1] - j + 1, ind_center[0] + i] + \
                          X_grid[2:4, ind_center[1] - j , ind_center[0] + i - 1] - \
                          X_grid[2:4, ind_center[1] - j + 1, ind_center[0] + i - 1]
                X_grid[2:4, ind_center[1] - j, ind_center[0] + i] = self.closest_point(x_point).T[0]
